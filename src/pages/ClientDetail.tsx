...
export function ClientDetail() {
  ...
  const planActivo =
    plans.find((p) => p.clientId === client.id && !p.archivado) ?? ensurePlan(client.id);
  // Al abrir una archivada desde el historial se edita esa; si no, la de uso.
  const plan = (verPlanId && plans.find((p) => p.id === verPlanId)) || planActivo;
    ...
    <div className="space-y-5">
      ...
      {plan.archivado && (
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2 text-xs text-amber-900 no-print">
          <span>
            Estás viendo <strong>{plan.nombre}</strong>, una planificación archivada. Los cambios que
            hagas aquí no son los que ve el cliente.
          </span>
          <button
      ...
      {tab === 'seguimiento' && (
        <AdherenceTab client={client} plan={plan} registros={registrosCliente} foods={foods} />
      )}

      {tab === 'antropometria' && (
        <AnthroTab client={client} onClientChange={(patch) => updateClient(client.id, patch)} />
      )}
                ...
                {plan.dayTypes.length > 1 && (
                  <button
                    onClick={() => {
                      ...
                      setDtIndex((idx) => Math.max(0, Math.min(idx, plan.dayTypes.length - 2)));
                    }}
                    aria-label={`Quitar ${d.nombre}`}
                    title={`Quitar ${d.nombre}`}
                    className={`px-2 py-1.5 text-xs transition ${
                      activo ? 'text-white/60 hover:text-white' : 'text-slate-300 hover:text-red-600'
                    }`}
                  >
                    ×
                  </button>
      ...
      {tab === 'plan' && (
        <div className="space-y-4">
          ...
          <SuggestedDistribution
            planeado={planeado}
            meals={dayType.meals}
            hayReparto={Object.values(dayType.grid).some((c) => Object.keys(c ?? {}).length > 0)}
      ...
      {tab === 'entrega' && (
        <div className="space-y-5">
              ...
              <div className="space-y-6">
                {dayType.meals.map((m) => (
                  <RecipeRecommender
                    ...
                    seleccionadas={recetasDeComida(dayType.recetasAsignadas, m.id)}
                    ...
                    onToggle={(rid) => {
                      ...
                      updateDayType(plan.id, dayType.id, {
                        recetasAsignadas: { ...(dayType.recetasAsignadas ?? {}), [m.id]: nuevas },
                      });
                    }}
                  />
                ))}
          ...
          {plan.fase === 3 && (
            <Card
              ...
              actions={
                <Input
                  ...
                  onChange={(e) => updateDayType(plan.id, dayType.id, { postre: e.target.value })}
            ...
            >
              <div className="space-y-2">
                <DayTemplateBar
                  dayType={dayType}
                  onDespensa={(despensa) => updateDayType(plan.id, dayType.id, { despensa })}
                />
                {dayType.meals.map((m) => (
                  <MealPantryEditor
                    key={m.id}
                    ...
                    onAceite={(porciones) =>
                      updateDayType(plan.id, dayType.id, {
                        aceiteCoccion: { ...(dayType.aceiteCoccion ?? {}), [m.id]: porciones },
                      })
                    }
                    onNota={(t) =>
          ...
          <SendPlanPanel
            ...
            onEnviar={(mensaje) =>
              updatePlan(plan.id, {
                envio: { fecha: new Date().toISOString(), mensaje: mensaje || undefined },
              })
            }
